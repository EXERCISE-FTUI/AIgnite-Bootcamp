...
async function main(): Promise<void> {
    ...
    const migrationFiles = fs
        ...
        .filter(
            file =>
                file.endsWith(".sql") &&
                fs.statSync(path.join(migrationsDir, file)).isFile()
        )
        .sort();

    if (migrationFiles.length === 0) {
        console.error("No SQL migration files found in migrations directory");
        process.exit(1);
    }

    console.log(
        `[migrate] Found ${migrationFiles.length} migration file(s):`,
        migrationFiles
    );

    // Load all migration contents
    const migrations: Array<{ filename: string; sql: string; lines: number }> =
        [];
    for (const filename of migrationFiles) {
        const migrationPath = path.join(migrationsDir, filename);
        const sql = fs.readFileSync(migrationPath, "utf-8");
        const sqlLines = sql.split(/\r?\n/).length;
        migrations.push({ filename, sql, lines: sqlLines });
        console.log(
            `[migrate] Loaded SQL from ${filename} (${sqlLines} lines)`
        );
    }
    ...
    try {
        ...
        for (const migration of migrations) {
            console.log(
                `[migrate] Executing migration: ${migration.filename}...`
            );
        ...
        console.log(
            `[migrate] All migrations completed successfully in ${durationMs} ms.`
        );
    } catch (err) {
        try {
            console.warn(
